...
function Dashboard(props) {
  ...
  const [isLoading, data, errorData] = useApiGet(api, "/user/dashboard", [])
	...
	let tips = [
		...
		<li>setting min-duration 0 will show all results (and overwrite max-duration) </li>,
		<li>salary is reset to 0 in case of invalid inputs (try typing letters!)</li>,		
	...
	const searchJobType = (content) =>{
		...
		let finaljobTypes = jobTypes.filter(jobType => jobType.includes(searchBarJob))
		let finalContent = []
		for (let i = 0; i < content.length; i++) {
			const element = content[i];
	...
	const searchSalary = (content) =>{
		...
		for (let i = 0; i < content.length; i++) {
			const element = content[i];
			if(salaryMin <= element.salary && element.salary <= salaryMax) finalContent.push(element)
		}
		return finalContent
	}
	...
	// searching for the time constraints
		const searchDuration = (content) =>{
			...
			for (let i = 0; i < content.length; i++) {
				...
				else if(durationMin < element.duration && element.duration < durationMax) finalContent.push(element)
			}
			return finalContent
		}
	...
	if(!isLoading && data){
		for (let i = 0; i < data.length; i++) {
			...
			// console.log('fuse wala content', content)
